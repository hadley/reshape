...
#' @export
add_margins <- function(df, vars, margins = TRUE, margin_label = "(all)") {
  ...
  addAll <- function(x) {
    x <- addNA(x, TRUE)
    factor(x, levels = c(levels(x), margin_label), exclude = NULL)
  }
  ...
  margin_dfs <- llply(margin_vars, function(vars) {
    df[vars] <- rep(list(factor(margin_label)), length(vars))
    df
  })
